...
def create_attributes(collection_id):
    attributes = [
        ...
        ("tags", "list_string"),  # Changed to list of strings
        ("score", "integer")
    ]
    ...
    for attr in attributes:
        try:
            if attr[1] == "integer":
                databases.create_integer_attribute(database_id, collection_id, attr[0], required=False)
            elif attr[1] == "string":
                ...
                databases.create_boolean_attribute(database_id, collection_id, attr[0], required=False)
            elif attr[1] == "list_string":
                databases.create_list_attribute(database_id, collection_id, attr[0], "string", required=False)  # List of strings
            p(f"Attribute {attr[0]} created successfully")
        except Exception as e:
            print(f"Error creating attribute {attr[0]}: {e}")

# Call the function to create attributes
